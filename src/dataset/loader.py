...
class EmotionBertDataset:
    ...
    def split_train_valid(self, seed=10, num=30000):
        data = self.__data[~self.__data["emotions"].isna()].reset_index(drop=True)
        indexes = data.index.tolist()
        import random
        random.seed(seed)
        random.shuffle(indexes)
        train_data = data.loc[indexes[:num]].reset_index(drop=True)
        valid_data = data.loc[indexes[num:]].reset_index(drop=True)
        return train_data, valid_data


class EmotionXgbDataset:
    def __init__(self, data_path):
        self.data_path = data_path
        self.__data = None
